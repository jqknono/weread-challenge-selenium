...
| `push` | 构建并推送 Docker image | 同 `build` | `docker build` 与逐个 `docker push` 执行结果 | Docker CLI、已登录的镜像仓库 | 仅推送显式给定的 tag 集合 |
